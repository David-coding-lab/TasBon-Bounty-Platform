...
export default function GithubCallback() {
  ...
  const [message, setMessage] = useState(
    'Please wait while we securely verify your GitHub account.',
  )
  ...
  useEffect(() => {
    async function handleGithubAuth() {
      try {
        ...
        setStatus('success')
        setMessage(
          'GitHub verification successful. Redirecting to dashboard...',
        )
      ...
      <div className="flex flex-col items-center justify-center text-center px-6 max-w-md">
        ...
        <div className="mb-8">
          <div className="w-16 h-16 rounded-2xl bg-black flex items-center justify-center shadow-lg">
            <span className="text-white font-bold text-xl font-sora">GH</span>
          </div>
        </div>
        ...
        <h1 className="text-xl md:text-2xl font-semibold text-gray-900 font-sora">
          {status === 'loading' && 'Verifying GitHub'}
