  ...
  return (
    <div className="flex flex-col space-y-6 bg-white min-h-full p-6">
      <div className="flex flex-col space-y-1">
        ...
        <p className="text-sm text-[#4A5565]">Track all the bounties you've applied for and their status.</p>
      </div>
      ...
      <div className="border border-[#E5E7EB] rounded-lg overflow-hidden">
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-4 px-4 py-3 bg-white border-b border-[#E5E7EB]">
          {['Bounty', 'Creator', 'Status', 'Applied On', 'Deadline', 'Reward'].map((col) => (
            <span key={col} className={`text-xs font-medium text-[#6B7280] uppercase tracking-wide ${col === 'Reward' ? 'text-right' : 'text-left'}`}>{col}</span>
          ))}
        </div>

        {loading ? (
          Array(5).fill(null).map((_, i) => (
            <div key={i} className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-4 px-4 py-4 border-b border-[#E5E7EB]">
